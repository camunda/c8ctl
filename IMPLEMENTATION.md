...
5. **Command Handlers** (`src/commands/`)
   ...
   - **Deployments** (`deployments.ts`): deploy with `_bb-` folder prioritization, duplicate ID validation, results table with file paths and building block indicators (🧱), and `.process-application` batch deployment detection
