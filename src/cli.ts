...
/**
 * Main CLI runner — accepts pre-sliced argv (no process.argv dependency)
 */
export async function runCli(argv: string[]) {
  ...
  if (verb === 'get' && normalizedResource === 'process-instance') {
    ...
    const includeVariables = argv.includes('--variables');
  ...
  if (verb === 'get' && normalizedResource === 'form') {
    ...
    // Check for flags and their aliases in the passed argv
    const isUserTask = argv.includes('--userTask') || argv.includes('--ut');
    const isProcessDefinition = argv.includes('--processDefinition') || argv.includes('--pd');
