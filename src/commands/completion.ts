...
_c8ctl_completions() {
  ...
  # Commands (verbs)
  local verbs="list search get create delete cancel await complete fail activate resolve publish correlate deploy run watch add remove rm load unload sync upgrade downgrade assign unassign init use which output completion help"
  ...
  local help_resources="list get create delete complete await search deploy run watch cancel resolve fail activate publish correlate upgrade downgrade init assign unassign profiles profile plugin plugins"

  # Global flags
  local flags="--help --version --profile --from --all --bpmnProcessId --id --processInstanceKey --processDefinitionKey --parentProcessInstanceKey --variables --state --assignee --type --correlationKey --timeToLive --maxJobsToActivate --timeout --worker --retries --errorMessage --baseUrl --clientId --clientSecret --audience --oAuthUrl --defaultTenantId --awaitCompletion --fetchVariables --requestTimeout --sortBy --asc --desc --limit --between --dateField --name --key --elementId --errorType --value --scopeKey --fullValue --userTask --ut --processDefinition --pd --iname --iid --iassignee --ierrorMessage --itype --ivalue --fields --dry-run --force --username --email --password --ownerId --ownerType --resourceType --resourceId --permissions --tenantId --claimName --claimValue --mappingRuleId --to-user --to-group --to-tenant --to-mapping-rule --from-user --from-group --from-tenant --from-mapping-rule"
